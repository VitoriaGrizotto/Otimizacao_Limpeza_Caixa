...
def buscar_caixa():
    ...
    resultado_label.config(text="❌ Caixa não encontrada", fg="#7f8c8d")
...
def gerar_e_mostrar_qrcode(codigo, data_limpeza_str):
    ...
    janela_qrcode.title(f"QR Code: Caixa {codigo}")
    ...
    tk.Label(main_frame_qr, text=f"QR Code para Caixa {codigo}",
    ...
    def salvar_qrcode_png():
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile=f"qrcode_caixa_{codigo}.png")
        if file_path:
    ...
    with open(CSV_FILE, 'r', newline='', encoding='utf-8') as csvfile:
        for row in csv.DictReader(csvfile):
            if row['codigo'] == codigo:
                messagebox.showwarning("Aviso", f"Caixa '{codigo}' já existe.")
                return
    if messagebox.askyesno("Gerar QR Code?", f"Deseja gerar QR Code para a caixa '{codigo}'?"):
...
def editar():
    global caixa_atual
    if not caixa_atual:
        messagebox.showwarning("Aviso", "Consulte uma caixa para editar.")
        return
    def salvar_edicao():
        ...
        with open(CSV_FILE, newline='', encoding='utf-8') as csvfile:
            for linha in csv.DictReader(csvfile):
                if linha['codigo'] == caixa_atual['codigo']:
        ...
        messagebox.showinfo("Sucesso", "Caixa editada com sucesso!")
        janela_edicao.destroy(); buscar_caixa(); gerar_relatorio()

    janela_edicao = tk.Toplevel(root)
    janela_edicao.title(f"✏️ Editar Caixa {caixa_atual['codigo']}")
    janela_edicao.configure(bg="#f4f6f9"); janela_edicao.geometry("500x350")
    janela_edicao.transient(root); janela_edicao.grab_set()

    tk.Label(janela_edicao, text=f"Editar Caixa {caixa_atual['codigo']}",
             font=("Segoe UI", 18, "bold"), bg="#f4f6f9", fg="#2c3e50").pack(pady=10)
    ...
    entrada_data_edit.insert(0, datetime.strptime(caixa_atual['data_ultima_limpeza'], "%Y-%m-%d").strftime("%d/%m/%Y"))
    ...
    entrada_hist_edit.insert(0, caixa_atual['historico'])
...
def gerar_relatorio():
    ...
    file_path = f"relatorios/relatorio_caixas_{proximo_mes:02d}_{ano_proximo}.txt"
    with open(file_path, "w", encoding="utf-8") as f:
        ...
        if caixas_proximo_mes:
            for c in caixas_proximo_mes:
                f.write(f"Caixa: {c['codigo']}\nÚltima: {c['ultima']}\nPróxima: {c['proxima']}\nHistórico: {c['historico']}\n")
                f.write("-"*50 + "\n")
        else:
...
tk.Label(root, text="Controle de Limpeza de Caixas",
