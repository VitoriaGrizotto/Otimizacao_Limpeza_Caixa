...
def gerar_e_mostrar_qrcode(codigo, data_limpeza_str):
    ...
    tk.Label(
        main_frame_qr,
        text=f"QR Code para Caixa {codigo}",
    ...
    tk.Label(
        main_frame_qr,
        text=f"Código: {codigo} | Data: {data_limpeza_str}",
        font=("Segoe UI", 12),
        bg="#f4f6f9",
        fg="#007bff",
    ).pack(pady=10)
...
def cadastrar():
    ...
    if messagebox.askyesno(
        "Gerar QR Code?", f"Deseja gerar QR Code para a caixa '{codigo}'?"
    ):
...
def editar():
    ...
    def salvar_edicao():
        ...
        with open(CSV_FILE, "w", newline="", encoding="utf-8") as csvfile:
            escritor = csv.DictWriter(
                csvfile, fieldnames=["codigo", "data_ultima_limpeza", "historico"]
            )
    ...
    tk.Label(
        janela_edicao,
        text=f"Editar Caixa {caixa_atual['codigo']}",
        font=("Segoe UI", 18, "bold"),
        bg="#f4f6f9",
        fg="#2c3e50",
    ).pack(pady=10)
    ...
    entrada_data_edit.insert(
        0,
        datetime.strptime(caixa_atual["data_ultima_limpeza"], "%Y-%m-%d").strftime(
            "%d/%m/%Y"
        ),
    )
...
def gerar_relatorio():
    hoje = datetime.today().date()
    proximo_mes, ano_proximo = (
        (1, hoje.year + 1) if hoje.month == 12 else (hoje.month + 1, hoje.year)
    )
    caixas_proximo_mes = []
    with open(CSV_FILE, newline="", encoding="utf-8") as csvfile:
        for linha in csv.DictReader(csvfile):
...
tk.Label(
    root,
    text="Controle de Limpeza de Caixas",
