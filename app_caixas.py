...
def buscar_caixa():
    ...
    if not encontrado:
        resultado.config(text="❌ Caixa não encontrada",
                         bg="#6c757d", fg="white")
    ...
    with open(CSV_FILE, 'r', newline='', encoding='utf-8') as csvfile:
        ...
        for row in leitor:
            if row['codigo'] == codigo:
                messagebox.showwarning("Aviso", f"Essa caixa '{codigo}' já existe.")
                return
...
def editar():
    global caixa_atual
    if not caixa_atual:
        messagebox.showwarning("Aviso", "Primeiro consulte uma caixa para editar.")
        return
    ...
    entrada_data_edit.insert(0, datetime.strptime(
        caixa_atual['data_ultima_limpeza'], "%Y-%m-%d").strftime("%d/%m/%Y"))
    ...
    estilo_botao(btn, "#ffc107", "#e0a800")
    btn.pack(pady=15, fill="x", padx=20)

    janela_edicao.bind('<Return>', lambda event=None: btn.invoke())
...
def focus_in(event):
    if entrada.get() == "Digite o código da caixa...":
...
btn1 = tk.Button(btn_frame, text="🔍 Consultar", command=buscar_caixa)
